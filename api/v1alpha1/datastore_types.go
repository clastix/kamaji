...
// +kubebuilder:validation:XValidation:rule="(self.driver != \"etcd\" && has(self.tlsConfig)) ? ((self.tlsConfig.certificateAuthority == null || (has(self.tlsConfig.certificateAuthority.certificate.secretReference) || has(self.tlsConfig.certificateAuthority.certificate.content)))) : true", message="When driver is not etcd and tlsConfig exists, certificateAuthority must be null or contain valid content"
